...
from Zara import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Zara/plugins/{shortname}.py")
        checkplugins(path)
        name = "Zara.plugins.{}".format(shortname)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"Zara.plugins.{shortname}"] = mod
        LOGS.info(f"تـم تثبيت ملـف {shortname}")
