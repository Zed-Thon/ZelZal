...
async def _(event):
    ...
    if channels_set == []:
        return
    already_sent = False  # متغير للتحقق من إرسال الرسالة
    for chat in channels_set:
        if event.media:
            if not already_sent:  # التحقق من عدم إرسال الرسالة بالفعل
                await event.client.send_file(int(chat), event.media, caption=event.text)
                already_sent = True  # تحديث قيمة المتغير بعد إرسال الرسالة
