...
async def mention(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return await edit_or_reply(mention, f"**⌔╎{zel_zal}**")
    ...
    await edit_or_reply(mention, f"**⌔╎لـ  ** [{zedth2}](tg://user?id={user.id}) \n**⌔╎{zel_zal}**")
...
async def zed(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(mention, f"**🚺 ╎ الحلـوه ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الكروب 🥳💃.**")
...
async def zed(mention):
    ...
    if not user:
        return
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(mention, f"**🚹 ╎ الحلـو  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه حبيبـج .. بنجـاح 💍🤵‍♂👰🏻‍♀.**")
