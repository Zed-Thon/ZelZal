...
UPSTREAM_REPO_BRANCH = "Zara"

REPO_REMOTE_NAME = "temponame"
IFFUCI_ACTIVE_BRANCH_NAME = "Zara"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/Zara"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/Zara", force=True)
    ...
    try:
        remote.push("Zara:main", force=True)
    except Exception as error:
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("Zara", origin.refs.Zara)
        repo.heads.Zara.set_tracking_branch(origin.refs.Zara)
        repo.heads.Zara.checkout(True)
