...
UPSTREAM_REPO_BRANCH = "Zars"

REPO_REMOTE_NAME = "temponame"
IFFUCI_ACTIVE_BRANCH_NAME = "Zars"
NO_HEROKU_APP_CFGD = "no heroku application found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/Zars"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        remote.push(refspec="HEAD:refs/heads/Zars", force=True)
    ...
    try:
        remote.push("Zars:main", force=True)
    except Exception as error:
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head("Zars", origin.refs.Zars)
        repo.heads.Zars.set_tracking_branch(origin.refs.Zars)
        repo.heads.Zars.checkout(True)
