...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الاستـخـدام": "{tr}ملصقات + كلمـة انكلـش",
    },
)
async def cb_sticker(event):
    ...
    if not split:
        return await edit_delete(event, "**- قم بادخـال اسم انكلـش للبحث عن حـزم ملصقـات . . .**", 5)
    ...
    if not results:
        return await edit_delete(zedevent, "**- لايوجـد نتائـج بحث :(**", 5)
    reply = f"**- نتائـج البحث عـن حـزم ملصقـات بـ اسـم {split} :**"
    for pack in results:
        if pack.button:
