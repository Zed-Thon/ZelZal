...
async def article_builder(event, method):
    ...
    title = "© ZThon Help"
    description = "Button menu for ZThon"
    if method == "deploy":
        media = "https://telegra.ph/file/f821d27af168206b472ad.mp4"
        title = "𝗭𝗧𝗵𝗼𝗻 𓅛"
        description = "روابـط التنصـيب"
        query = "- لـ تنصيب سـورس زدثــون 𝗭𝗧𝗵𝗼𝗻"
        buttons = [
            (
                Button.url("مطـور السـورس", "https://t.me/zzzzl1l"),
                Button.url("قنـاة السـورس", "https://t.me/ZedThon/105"),
            )
        ]
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("Inline buttons"):
