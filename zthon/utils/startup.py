    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                ...
                caption="**•⎆┊تـم بـدء تشغـيل سـورس افتار الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𖣳𝙎𝙊𝙐𝙍𝘾𝞝𖢻𝘼𝙑𝘼𝙏𝘼𝙍𖣳", "https://t.me/source_av"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت افتار المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "اشعارات افتار", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "تخزين افتار", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
