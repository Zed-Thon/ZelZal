...
async def _(event):
    ...
    if response_api := requests.get(sample_url).text:
        await edit_or_reply(event, f"**- الدوميـن الخـاص بالرابـط** {input_str} **هـو** \n{response_api}")
    else:
        await edit_or_reply(
...
async def _(event):
    ...
    if response_api := requests.get(sample_url).text:
        await edit_or_reply(
            event, f"**- تم انشـاء الإختصـار** {response_api} **لـ الرابـط** {input_str} **.. بنجـاح**", link_preview=False
        )
    else:
