...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗭𝗧𝗛𝗢𝗡 - الـرد التلقـائي 〽️](t.me/ZedThon) .
**•─────────────────•**

❞ **مرحبـاً**  {mention} ❝

**⤶ قد اكـون مشغـول او غيـر موجـود حـاليـاً ؟!**
**⤶ ❨ لديـك** {warns} **تحذيـرات فقـط ⚠️❩**
        ...
        if gvarstatus("pmmenu") is None:
            ...
            if CAT_IMG is not None:
                ...
                msg = await event.client.send_message(
                    chat.id, USER_BOT_NO_WARN, reply_to=reply_to_id, link_preview=False
                )
    except Exception as e:
        LOGS.error(e)
...
async def on_new_private_message(event):
    ...
    if event.chat_id in zel_dev:
        ...
        return await event.client.send_message(chat, "**احد المطورين المساعدين هنـا اننـي محظـوظ لقدومـك الـي 🙈♥️**")
