...
zel_dev = (1400467850, 627658332, 1050898456, 5881570606, 6161236633, 5252385385, 5607917250)
...
async def zedgban(event):  # sourcery no-metrics
    ...
    if user.id == 5190136458 or user.id == 1355571767:
        return await edit_delete(zede, "**⎉╎عـذراً ..لا استطيـع حظـࢪ مبرمج السـورس عـام **")
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        ifif user.id == 5190136458 or user.id == 1355571767:
            return await edit_or_reply(event, "**- عــذࢪاً .. لايمكــنك كتــم مبرمج السورس عــام ؟!**")
        userid = user.id
    try:
...
async def catgkick(event):  # sourcery no-metrics
    ...
    ifif user.id == 5190136458 or user.id == 1355571767:
        return await edit_delete(zede, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد مبرمج السورس ... ❏╰**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
