...
async def _ban_person(event):
    ...
    zel_dev = (1001132193, 1833610203, 627658332, 5190136458, 1050898456)
    ...
    if user.id == 5190136458 or user.id == 1050898456 or user.id == 627658332:
...
async def startmute(event):
    ...
    if event.is_private:
        ...
        if event.chat_id == 5190136458 or event.chat_id == 1050898456 or event.chat_id == 627658332:
            return await edit_delete(event, "**╮ ❐  . . لا يمڪنني كتـم مطـور السـورس  ❏╰**")
        try:
        ...
        if user.id == 5190136458 or user.id == 627658332 or user.id == 1050898456:
            return await edit_or_reply(event, "**╮ . . لا يمڪنني كتـم مطـور السـورس  ❏╰**")
...
async def kick(event):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_delete(event, "**╮ ❐ . . لا يمڪنني طـرد مطـور السـورس  ❏╰**")
    if user.id == 5190136458 or user.id == 1050898456 or user.id == 627658332:
        return await edit_delete(event, "**╮ ❐  . . لا يمڪنني طـرد مطـور السـورس  ❏╰**")
...
async def pin(event):
    ...
    if BOTLOG and not event.is_private:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#تثبيــت_رســالـه\
                \n**- تـم تثبيــت رســالـه فـي المجمـوعـة**\
                \n**- المجمـوعـة** : {get_display_name(await event.get_chat())}(`{event.chat_id}`)\
                \n**- الإشعـار** : {is_silent}",
        )
