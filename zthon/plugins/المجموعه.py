...
async def tagall(event):
  ...
  if event.is_private:
    return await event.respond("**- عـذراً ... هـذه ليـست مجمـوعـة ؟!**")
  if event.pattern_match.group(1):
    ...
    if msg == None:
        return await event.respond("**- عـذراً ... الرسـالة غيـر ظـاهـرة للأعضـاء الجـدد ؟!**")
  elif event.pattern_match.group(1) and event.reply_to_msg_id:
    return await event.respond("**- اضـف نـص لـ الامـر . . .**\n\n**- مثـال :** `.all وينكـم`")
  else:
    return await event.respond("**- بالـرد عـلى رسـالـه . . او باضـافة نـص مـع الامـر**")
  if mode == "by_cmd":
    ...
    async for usr in zedub.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        await event.respond("**- تم إيقـاف تـاك all**")
        return
      if usrnum == 5:
...
async def stop_tagall(event):
  if not event.chat_id in moment_worker:
    return await event.respond('**- عـذراً .. لا يوجـد هنـاك تـاك لـ إيقـافـه ؟!**')
  else:
    try:
      moment_worker.remove(event.chat_id)
    except:
      pass
    return await event.respond('**- تم إيقـاف تـاك all .. بنجـاح ✓**')
...
async def _(event):  # sourcery no-metrics
    ...
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n += 1
    if input_str:
        required_string = """𓆰 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝𝘿 - 🌐 - 𝑮𝑹𝑼𝑶𝑷 𝑺𝑻𝑨𝑻𝑺 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻 
...
@zedub.zed_cmd(pattern="كروباتي$")
async def _(event):
    if event.fwd_from:
        return
    result = await bot(GetAdminedPublicChannelsRequest())
    output_str = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝𝘿 - 🌐 - **احصـائيـات الدردشـات** 𓆪\n\n"
    for channel_obj in result.chats:
        output_str += f"**⪼ الدردشـة ↩︎** {channel_obj.title} @{channel_obj.username} .\n"
    await event.edit(output_str)

@zedub.zed_cmd(pattern="دردشاتي$")
async def _(event):
    if event.fwd_from:
        return
    result = await bot(GetAdminedPublicChannelsRequest())
    output_str = "𓆩 𝙎𝙊𝙐??𝘾𝞝 𝙕𝞝𝘿 - 🌐 - **احصـائيـات الدردشـات** 𓆪\n\n"
    for channel_obj in result.chats:
        output_str += f"**⪼ الدردشـة ↩︎** {channel_obj.title} @{channel_obj.username} .\n"
    await event.edit(output_str)
