...
zel_dev = (5289124437, 627658332, 5190136458, 1050898456, 1355571767)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗩𝗘𝗡𝗢𝗠 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/q_k_2) 🗑♻️𓆪\n**𓍹━─━─━𝗩𝗘𝗡𝗢𝗠━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐  . . هـذا احـد المطـورين السورس  ❏╰**")
    if user.id == 1355571767 or user.id == 627658332 or user.id == 1050898456:
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐  . . هـذا احـد المطـورين السورس  ❏╰**")
    if user.id == 1355571767 or user.id == 1050898456 or user.id == 627658332:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗩𝗘𝗡𝗢𝗠 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/q_k_2) 𓆪\n\n"
