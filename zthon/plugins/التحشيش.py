...
@l313l.ar_cmd(pattern="رابط الحذف")
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/S_EG_P) 🗑♻️𓆪\n**𓍹━─━─━─━─𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")

########################  SOURCE ZED ~ BY: ZelZal (@zzzzl1l)  ########################

@l313l.ar_cmd(pattern="رفع جلب(?: |$)(.*)")
async def permalink(mention):
    ...
    if user.id == 627658332 or user.id == 5190136458 or user.id == 1050898456:
...
@l313l.ar_cmd(pattern="رفع جريذي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
ZelzalTHS_cmd = (
    "𓆩 [𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴 - اوامـر التحشيش](t.me/S_EG_P) 𓆪\n\n"
...
@l313l.ar_cmd(pattern="(نسبه الحب|نسبة الحب)(?: |$)(.*)")
...
@l313l.ar_cmd(pattern="(نسبه الانوثة|نسبة الانوثه|نسبه الانوثه|نسبة الانوثة)(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="(نسبه الغباء|نسبة الغباء)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="(نسبه الانحراف|نسبة الانحراف)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="(نسبه المثليه|نسبة المثليه)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="(نسبه النجاح|نسبة النجاح)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="(نسبه الكراهية|نسبة الكراهيه|نسبه الكراهيه|نسبة الكراهية)(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def zed(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{zedth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه خـروف 🐑.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
@l313l.ar_cmd(pattern="رفع حيوان(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="رفع زباله(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="رفع مرتبط(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="رفع مرتبطه(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="رفع حبيبي(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@l313l.ar_cmd(pattern="رفع خطيبتي(?: |$)(.*)")
async def zed(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين المساعديـن  ❏╰**")
    ifif user.id == 627658332 or user.id == 5190136458 or user.id == 1050898456:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
