...
zel_dev = ( 1001132193, 1833610203, 627658332, 5190136458, 1050898456, 1355571767)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙏𝙀𝙇𝙀𝙃𝙊𝙉☥𝘾𝙍 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/S_EG_P) 🗑♻️𓆪\n**⎈•━━⊶𐂡•𝙏𝙀𝙇𝙀𝙃𝙊𝙉☥𝘾𝙍•𐂡⊷━━•⎈**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ ده . . من اصحاب كوكب كرستين  ❏╰**")
...
ZelzalTHS_cmd = (
    "𓆩 [ŜỖǗŘČẸ ČŘĮŜŤĮŇ 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/S_EG_P) 𓆪\n\n"
...
async def zed(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐  هـذا مطـور السورس . . مهما قولت مش هعرف اوصف جمدانه 🙊💘 ❏╰**")
    if user.id == 5190136458 or user.id == 1050898456 or user.id == 627658332:
        return await edit_or_reply(mention, f"**╮ ❐  هـذا مطـور السورس . . مهما قولت مش هعرف اوصف جمدانه 🙊💘 ❏╰**")
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
