...
async def get_users(event):
    ...
    restricted = ["@zthon_support", "@zedthon_support"]
    zedub = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if zedub_chat in restricted:
        return await zedub.edit(
            event, "..."
        )
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await zedub.edit("**❈╎ جـارِ إتمـام العمليـة إنتظـر ⅏ . . .**")
    else:
        await zedub.edit("**❈╎ جـارِ إتمـام العمليـة إنتظـر ⅏ . . .**")
    if event.is_private:
        return await zedub.edit("**╮  لا استطـيع اضافـة الاعضـاء هـنا 𓅫╰**")
    s = 0
    f = 0
    error = "None"
    await zedub.edit("**❈╎حالة الأضافة:**\n\n**❈╎تتم جمع معلومات المستخدمين 🔄 ...⏣**")
