...
async def noods(event):  # sourcery no-metrics
    ...
    zedevent = await edit_or_reply(event, "🔍 `Searching...`")
    r = requests.get(
    ...
    piclinks = []
    captionlist = []
    await edit_or_reply(zedevent, "⏳ `Processing..`")
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            await edit_or_reply(zedevent, "📥** Downloading...**")
            ...
            i += 1
        await edit_or_reply(
            zedevent, f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**"
        )
        if count == int(limit):
            break
        if i == 5:
    ...
    try:
        await edit_or_reply(zedevent, "`Sending...`")
