...
async def chat_blacklist(event):
    ...
    if input_str == "تفعيل":
        ...
        if len(sudousers) != 0:
            text += (
                "**✾╎يتم الان اعـادة تشغيـل بـوت زدثــون انتظـر 2-1 دقيقـه ▬▭ ...**"
            )
...
@zedub.zed_cmd(
    pattern="تعيين اوامر(s)?(?:\s|$)([\s\S]*)",
    command=("تعيين اوامر", plugin_category),
    info={
