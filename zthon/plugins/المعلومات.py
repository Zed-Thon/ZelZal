...
async def _(event):
    ...
    OUTPUT = f"**⌔∮ [𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴](tg://need_update_for_some_feature/) , تم حساب سرعة السيرفر:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت كرستين - 𓆩𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴𓆪**\n\n"
