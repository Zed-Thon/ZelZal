...
async def _(event):
    ...
    OUTPUT = f"**⌔∮ [𖣳𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂𖣳](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᯓ 𖣳𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂𖣳](tg://need_update_for_some_feature/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت روزا - 𓆩𖣳𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂𖣳𓆪**\n\n"
