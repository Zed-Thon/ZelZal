...
async def _(event):
    ...
    OUTPUT = f"**⌔∮ [𝗦𝗢𝗨𝗥𝗖𝗘 𝐀𝐕𝐀𝐓𝐀𝐑 𓅛](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝐀𝐕𝐀𝐓𝐀𝐑 𓅛](tg://need_update_for_some_feature/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐀𝐕𝐀𝐓𝐀𝐑 𓅛](tg://need_update_for_some_feature/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت زدثـــون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝐀𝐕𝐀𝐓𝐀𝐑 𓅛𓆪**\n\n"
