...
ZelzalVP_cmd = (
    "𓆩 [ᴧꝛᴛᴇx • ˢᴼᵁᴿᶜᴱ 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/pp_g3) 𓆪\n\n"
    ...
    "\n𓆩 [ᴧꝛᴛᴇx 𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/pp_g3) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ᴧꝛᴛᴇx يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الان اعـادة تشغيـل بـوت ᴧꝛᴛᴇx يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def variable(event):
    ...
    if input_str == "العراق" or input_str == "عراق":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**⎉╎يتم الان اعـادة تشغيـل بـوت ᴧꝛᴛᴇx يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
