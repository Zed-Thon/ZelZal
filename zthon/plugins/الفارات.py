...
async def variable(event):
    ...
    if input_str == "رمز الوقتي" or input_str == "رمز الاسم الوقتي":
        ...
        await asyncio.sleep(1.5)
        if gvarstatus("MAX_FLOOD_IN_PMS") is None:
            await zed.edit("**✾╎الفـار {} غيـر موجـود ❌**\n**✾╎المتغيـر : ↶** لا يوجـد \n\n**✾╎قنـاة السـورس : @ZEDThon**".format(input_str))
        else:
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس :  @ZEDThon**".format(input_str, gvarstatus("MAX_FLOOD_IN_PMS")))
        ...
        if variable in heroku_var:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس :  @ZEDThon**".format(input_str, gvarstatus("pmpermit_pic")))

    elif input_str == "صورة الوقتي" or input_str == "صوره الوقتي" or input_str == "البروفايل تلقائي":
        variable = "DIGITAL_PIC"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎المطـور {} موجـود 🧑🏻‍💻☑️**\n**✾╎ايدي المطـور : ↶**\n `{}` \n**✾╎قنـاة السـورس : @ZEDThon**".format(input_str, heroku_var[variable]))
        ...
        if gvarstatus("ALIVE_PIC") is None:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس :  @ZEDThon**".format(input_str, gvarstatus("ALIVE_PIC")))
        ...
        if gvarstatus("ALIVE_TEMPLATE") is None:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس :  @ZEDThon**".format(input_str, gvarstatus("ALIVE_TEMPLATE")))
        ...
        if gvarstatus("pmpermit_txt") is None:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس :  @ZEDThon**".format(input_str, gvarstatus("pmpermit_txt")))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@zedub.zed_cmd(pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي) ?(.*)")
async def _(malatha):
    if malatha.fwd_from:
        return
    if Config.HEROKU_API_KEY is None:
