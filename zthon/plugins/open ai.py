...
async def song2(event):
    ...
    chat = "@CHATGPTAI_TG_BOT" # code by t.me/zzzzl1l
    reply_id_ = await reply_id(event)
    zed = await edit_or_reply(event, f"**⎉╎جـارِ جلب رد   ...**\n**زين مبرمج السورس**")
    async with event.client.conversation(chat) as conv:
        try:
            gool = "/chatgpt {}".format(been)
            await conv.send_message(gool)
        except YouBlockedUserError:
            await zedub(unblock("CHATGPTAI_TG_BOT"))
            gool = "/chatgpt {}".format(been)
            await conv.send_message(gool)
...
@zedub.zed_cmd(pattern="كريس(?:\s|$)([\s\S]*)")
async def song2(event):
    been = event.pattern_match.group(1)
    chat = "@CHATGPTAI_TG_BOT" # code by t.me/zzzzl1l
    reply_id_ = await reply_id(event)
