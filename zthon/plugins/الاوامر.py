...
async def zed_handler(event):
    ...
    if query.startswith("زدثون") and event.query.user_id == zedub.uid:
        ...
        if ZEDPIC and ZEDPIC.endswith((".jpg", ".png", "gif", "mp4")):
            result = builder.photo(ZEDPIC,text=Channels, buttons=buttons, link_preview=True)
        elif ZEDPIC:
            result = builder.document(ZEDPIC,title="zedub",text=Channels,buttons=buttons,link_preview=True)
        else:
