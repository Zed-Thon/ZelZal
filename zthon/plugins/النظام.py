...
@zedub.zed_cmd(
    pattern="تاريخ التنصيب$",
    command=("تاريخ التنصيب", plugin_category),
    info={
        "header": "To show deploy History",
        "الاستـخـدام": "{tr}تاريخ التنصيب",
    },
)
async def zeddd(event):
    uname = platform.uname()
    zedt = "**𓆪 تاريخ تنصيبـك لـ بـوت زدثـــون𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝𝘿**"
    boot_time_timestamp = psutil.boot_time()
    zz = datetime.fromtimestamp(boot_time_timestamp)
    zedt += f"` {zz.year}/{zz.month}/{zz.day} `"
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        svmem = psutil.virtual_memory()
