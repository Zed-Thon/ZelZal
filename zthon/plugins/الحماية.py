...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5190136458:
        await event.client.send_message(chat, "- اهلا بك يا زين الهقر مبرمج السورس 🖤 ")
    if event.chat_id == 627658332:
        await event.client.send_message(chat, "- اهلا بك يا مانو الشيطان 🖤 ")
    if event.chat_id == 1050898456:
        await event.client.send_message(chat, "- اهلا بك يا كرستين صاحبة السورس يولاه  🖤 ")        
...
@zedub.zed_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق جمثون
...
@zedub.zed_cmd(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
...
@zedub.zed_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)")
...
@zedub.zed_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@zedub.zed_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
# ترجمه وكتابة فريق جمثون
@zedub.zed_cmd(pattern="المسموح لهم$")
async def approve_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
