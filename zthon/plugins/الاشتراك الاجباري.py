        ...
        try:
            ...
            try:
                if p.title:
                    await asyncio.sleep(1.5)
                    delgvar("Custom_Pm_Channel")
                    addgvar("Custom_Pm_Channel", p.id)
                    return await edit_or_reply(
                        event, f"**✾╎تم إضافة قناة الاشتراك الاجباري للخاص .. بنجـاح ☑️**\n\n**✾╎اسم القناة : ↶** `{p.title}`\n**✾╎ايدي القناة : ↶** `{p.id}`\n\n**✾╎ارسـل الان** `.اشتراك خاص`"
                    )
            except Exception as e:
...
@zedub.zed_cmd(pattern="(ضع اشتراك كروب|وضع اشتراك كروب)(?:\s|$)([\s\S]*)")
async def _(event):
    if input_str := event.pattern_match.group(2):
        try:
            p = await event.client.get_entity(input_str)
...
async def supc(event):
    ...
    if ty == "كروب" or ty == "جروب" or  ty == "قروب" or  ty == "مجموعة" or  ty == "مجموعه":
        ...
        if event.is_group:
            ...
            addgvar("sub_group", f"{event.chat_id}")
...
async def supc (event):
    ...
    if cc == "كروب" or ty == "جروب" or  ty == "قروب" or  ty == "مجموعة" or  ty == "مجموعه" or cc == "الكروب" or cc == "اشتراك الكروب":
