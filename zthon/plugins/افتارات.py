...
async def _(event):
    ...
    if not reply_to_id:
        return await edit_or_reply(
            event, "**╮ قـم باضافـة رقمـٓہ من 130 - 1 للامـر . . مثـال ( .ر 1 ) ...𓅫╰**"
        )
    chat = "@SSSS_sssiBOT"
    zzevent = await edit_or_reply(event, "**╮•⎚ جـارِ تحميـل الافتـار ...**")
    async with event.client.conversation(chat) as conv:
...
async def _(event):
    ...
    if not reply_to_id:
        return await edit_or_reply(
            event, "**╮ قـم باضافـة رقمـٓہ من 29 - 1 للامـر . . مثـال ( .ت 1 ) ...𓅫╰**"
        )
    chat = "@Zedthonbot"
...
async def _(event):
    ...
    if not reply_to_id:
        return await edit_or_reply(
            event, "**╮ قـم باضافـة رقمـٓہ من 57 - 1 للامـر . . مثـال ( .ح 1 ) ...𓅫╰**"
        )
    chat = "@ZlZZl777BOT"
    zzevent = await edit_or_reply(event, "**╮•⎚ جـارِ التحميل ... 🧸🎈**")
    async with event.client.conversation(chat) as conv:
