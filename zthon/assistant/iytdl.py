...
async def iytdl_inline(event):
    ...
    if not input_url:
        return await edit_delete(event, "**يجب عليك الرد على الرابط او كتابة عنوان مع الامر**")
    zedevent = await edit_or_reply(event, f"🔎 جار البحث في اليوتيوب: `'{input_url}'`")
    flag = True
    cout = 0
    results = None
    while flag:
    ...
    if results:
        ...
        await zedevent.edit("**- عًذراً .. لم اجد اي نتائـج**")
...
async def ytdl_download_callback(c_q: CallbackQuery):  # sourcery no-metrics
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        await edit_delete(upload_msg, "لـم يتـم ايجـاد المطلـوب ؟!")
        return
    if not thumb_pic:
