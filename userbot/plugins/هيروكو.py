...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        zed = await edit_or_reply(var, "`Getting information...`")
        ...
        zed = await edit_or_reply(var, "`Setting information...`")
        if not variable:
        ...
        if variable in heroku_var:
            await zed.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await zed.edit(
        ...
        zed = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await zed.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await zed.edit(f"`{variable}`**  does not exist**")

        await zed.edit(f"`{variable}`  **successfully deleted**")
