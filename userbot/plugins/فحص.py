...
async def amireallyalive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    cat_caption = "**ZThon is Up and Running**\n"
    cat_caption += f"**{Z_EMOJI} Telethon version :** `{version.__version__}\n`"
    cat_caption += f"**{Z_EMOJI} Catuserbot Version :** `{zedversion}`\n"
    cat_caption += f"**{Z_EMOJI} Python Version :** `{python_version()}\n`"
    cat_caption += f"**{Z_EMOJI} Master:** {mention}\n"
