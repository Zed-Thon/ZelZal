...
async def download_audio(event):
    ...
    zedevent = await edit_or_reply(event, "`Preparing to download...`")
...
async def insta_dl(event):
    ...
    if not link and reply:
        link = reply.text
    if not link:
        return await edit_delete(event, "**à² âà²  Give me link to search..**", 10)
    ...
    v2 = "@videomaniacbot"
    media_list = []
    zedevent = await edit_or_reply(event, "**Downloading.....**")
