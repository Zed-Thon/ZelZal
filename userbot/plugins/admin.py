...
async def kick(event):
    ...
    if not user:
        return
    zedevent = await edit_or_reply(event, "`Kicking...`")
    try:
    ...
    if reason:
        ...
        await zedevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    zedevent = await edit_or_reply(event, "`Searching recent actions .....`")
