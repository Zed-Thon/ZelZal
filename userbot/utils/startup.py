        ...
        for option in config.dc_options:
            if option.ip_address == zedub.session.server_address:
                if zedub.session.dc_id != option.id:
                    LOGS.warning(
                        f"Fixed DC ID in session from {zedub.session.dc_id}"
                        f" to {option.id}"
                    )
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "لا تقم بحذف هذه المجموعة أو التغيير إلى مجموعة عامه (إذا قمت بتغيير المجموعة ، فسيتم فقدان كل البيانات السابقة.)"
        _, groupid = await create_supergroup(
            "كـروب التخـزين زدثـــون", zedub, Config.TG_BOT_USERNAME, descript
        )
